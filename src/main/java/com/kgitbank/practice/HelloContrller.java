...
public class HelloContrller {
	...
	*/
	...
	public String choo(
		...
		//파라미터에 실려온값이 매개변수에 들어있고, 모델 어트리뷰트에도 자동으로 실려있다
	    ...
	    @GetMapping("/kim")
	    public String kim() {
	    	//앞에 redirect:을 붙히면 뷰를 찾지않고 리다이렉트를 응답한다
	    ...
	    public ModelAndView modelAndView() {
	    	...
	    	//setViewName으로 다음으로 포워드 할 페이지까지 선택할수있는 만능객체
		...
		public String joinPage(HttpServletRequest request) {
			...
			System.out.println("내가 파일을 써야하는 경로:"
	    ...
	    public String joinProcess(Member mem) {
	    	...
	    	if(mem.nullExist()) {
	    		log.error("회원정보에 null 값이 존재함");
	    		return "redirect:/";//에러페이지로 포워드걸면 됨
	    	}
